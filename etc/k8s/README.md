...
Necessary helm charts are created under `etc/k8s/financeservices` folder. Based on your back-office application UI, remove the other UI charts. Ex, if you are using **angular** UI, remove **web**, **blazor** and **blazor-server** charts.
...
to generate pfx file for signing tokens by AuthServer. Edit `MyCompanyName.FinanceServices.AuthServer.csproj` file and uncomment item group section related to authserver.pfx:
...
```powershell
127.0.0.1 financeservices-st-web
127.0.0.1 financeservices-st-public-web
127.0.0.1 financeservices-st-authserver
127.0.0.1 financeservices-st-identity
127.0.0.1 financeservices-st-administration
127.0.0.1 financeservices-st-product
127.0.0.1 financeservices-st-saas
127.0.0.1 financeservices-st-gateway-web
127.0.0.1 financeservices-st-gateway-web-public
```
...
-   Run `deploy-staging.ps1` in the `helm-chart` directory. It is deployed with the `financeservices` namespace.

-   _You may wait ~30 seconds on first run for preparing the database_.

-   Browse https://financeservices-st-public-web for public and
    https://financeservices-st-web for the back-office application
